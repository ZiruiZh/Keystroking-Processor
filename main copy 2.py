...
class XMLToVideoApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Typing Analysis Workstation")
    ...
    def create_widgets(self):
        ...
        title_frame = tk.Frame(self.main_container, bg=self.colors['bg_main'])
        title_frame.pack(fill="x", padx=20, pady=(20, 10))
        
        title_label = tk.Label(title_frame, text="Typing Analysis Workstation", 
                               font=self.fonts['title'], bg=self.colors['bg_main'], 
                               fg=self.colors['text_primary'])
        title_label.pack(anchor="w")
        
        subtitle_label = tk.Label(title_frame, text="Transform typing data into video visualizations", 
                                  font=self.fonts['small'], bg=self.colors['bg_main'], 
                                  fg=self.colors['text_secondary'])
        subtitle_label.pack(anchor="w", pady=(5, 0))

        # File type selector card (fixed below title)
        file_type_card = tk.Frame(self.main_container, bg=self.colors['bg_card'], relief="flat", bd=0)
        file_type_card.pack(fill="x", padx=20, pady=(0, 10))
        
        # Add subtle border effect
        border_frame = tk.Frame(file_type_card, bg=self.colors['border'], height=1)
        border_frame.pack(fill="x", side="bottom")
        
        file_type_inner = tk.Frame(file_type_card, bg=self.colors['bg_card'])
        file_type_inner.pack(fill="x", padx=20, pady=15)
        
        tk.Label(file_type_inner, text="File Type", font=self.fonts['heading'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).pack(side="left", padx=(0, 15))
        
        self.file_type_var = tk.StringVar(value="XML/Word")
        self.file_type_dropdown = ttk.Combobox(file_type_inner, textvariable=self.file_type_var, 
                                               values=["XML/Word", "Data TXT", "IDFX"], 
                                               state="readonly", width=18, font=self.fonts['body'])
        self.file_type_dropdown.pack(side="left")
        self.file_type_dropdown.bind("<<ComboboxSelected>>", self.on_file_type_change)

        # Style ttk widgets
        style = ttk.Style()
        style.theme_use('clam')
        style.configure('TCombobox', fieldbackground=self.colors['bg_card'], 
    ...
    def create_scrollable_frame(self):
        ...
        canvas_frame = tk.Frame(self.main_container, bg=self.colors['bg_main'])
        canvas_frame.pack(fill="both", expand=True, padx=20)
        
        # Canvas for scrolling (make it focusable for touchpad support)
        self.canvas = tk.Canvas(canvas_frame, bg=self.colors['bg_main'], highlightthickness=0, 
                               takefocus=True)
        scrollbar = tk.Scrollbar(canvas_frame, orient="vertical", command=self.canvas.yview)
        
        # Scrollable frame inside canvas
        self.main_frame = tk.Frame(self.canvas, bg=self.colors['bg_main'])
        ...
        # Mouse wheel and touchpad scrolling (cross-platform)
        def on_mousewheel(event):
            # Windows and Mac with mouse/touchpad
            if hasattr(event, 'delta') and event.delta:
                delta = event.delta
                # Calculate scroll amount based on delta
                # Touchpads send smaller deltas, mouse wheels send larger (multiples of 120)
                if abs(delta) >= 120:
                    # Mouse wheel - standard behavior
                    scroll_amount = int(-delta / 120)
                else:
                    # Touchpad - handle small deltas directly
                    # For very small deltas (1-5), scroll by 1 unit
                    # For larger touchpad deltas, scale appropriately
                    if abs(delta) <= 5:
                        scroll_amount = -1 if delta < 0 else 1
                    else:
                        # Scale the delta for responsive scrolling
        ...
        def on_canvas_click(event):
            # Focus canvas when clicked to enable touchpad scrolling
            self.canvas.focus_set()
        
        def on_canvas_enter(event):
            # Focus canvas when mouse enters to enable touchpad scrolling
            self.canvas.focus_set()
        
        # Bind click and enter events to focus canvas
        self.canvas.bind("<Button-1>", on_canvas_click)
        self.canvas.bind("<Enter>", on_canvas_enter)
        # Also bind to the inner frame
        self.main_frame.bind("<Button-1>", on_canvas_click)
        self.main_frame.bind("<Enter>", on_canvas_enter)
        
        # Bind mouse wheel events globally (only once to prevent double processing)
        # Using bind_all ensures we catch all scroll events without duplicates
        self.canvas.bind_all("<MouseWheel>", on_mousewheel)
        self.canvas.bind_all("<Button-4>", on_mousewheel)
        self.canvas.bind_all("<Button-5>", on_mousewheel)
        
        # Try alternative event bindings for macOS trackpad
        try:
            self.canvas.bind_all("<Shift-MouseWheel>", on_mousewheel)
        except:
            pass
        
        # Enable canvas to receive focus for scrolling
        self.canvas.focus_set()
        
        # Ensure canvas gets focus when window is activated (helps with touchpad on macOS)
        def on_window_activate(event=None):
            self.canvas.focus_set()
        
        self.root.bind("<FocusIn>", on_window_activate)
        self.root.bind("<Map>", on_window_activate)  # When window is mapped/shown
        ...
        # Store reference for updating scroll region
        self.scrollbar = scrollbar
        
        # Create frames for each file type with modern styling
        self.xml_frame = tk.Frame(self.main_frame, bg=self.colors['bg_main'])
        self.data_frame = tk.Frame(self.main_frame, bg=self.colors['bg_main'])
        self.idfx_frame = tk.Frame(self.main_frame, bg=self.colors['bg_main'])
        ...
        # Create all file type frames
        self.create_file_type_frames()
        
        # Create shared settings section
    ...
    def create_modern_button(self, parent, text, command, bg_color=None, width=None):
        ...
        btn = tk.Button(parent, text=text, command=command, 
                       ...
                       activebackground=self.colors['bg_primary_hover'],
    ...
    def create_file_type_frames(self):
        # --- XML/Word Frame ---
        self.create_xml_frame()
        self.create_data_frame()
        self.create_idfx_frame()

    def create_xml_frame(self):
        """Create modern XML/Word file frame"""
        # Main card container
        xml_card = tk.Frame(self.xml_frame, bg=self.colors['bg_card'], relief="flat")
        xml_card.pack(fill="both", expand=True, pady=(0, 15))
        
        # Card header
        header_frame = tk.Frame(xml_card, bg=self.colors['bg_card'], height=50)
        header_frame.pack(fill="x", padx=20, pady=(15, 10))
        header_frame.pack_propagate(False)
        
        tk.Label(header_frame, text="XML/Word Files", font=self.fonts['heading'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).pack(side="left", anchor="w")
        
        self.xml_queue_label = tk.Label(header_frame, text="0 files in queue", 
                                        font=self.fonts['small'], bg=self.colors['bg_card'], 
                                        fg=self.colors['text_secondary'])
        self.xml_queue_label.pack(side="right", anchor="e")
        
        # Controls section
        controls_frame = tk.Frame(xml_card, bg=self.colors['bg_card'])
        controls_frame.pack(fill="x", padx=20, pady=(0, 15))
        
        self.add_xml_btn = self.create_modern_button(controls_frame, "Add Files to Queue", 
                                                    self.add_xml_to_queue, 
                                                    bg_color=self.colors['bg_primary'])
        self.add_xml_btn.pack(side="left", padx=(0, 10))
        
        clear_btn = tk.Button(controls_frame, text="Clear Queue", command=self.clear_xml_queue,
                            font=self.fonts['body'], bg=self.colors['bg_card'], 
                            fg=self.colors['text_secondary'], activebackground=self.colors['bg_main'],
                            activeforeground=self.colors['text_primary'], relief='flat', bd=0,
                            padx=15, pady=10, cursor='hand2')
        clear_btn.pack(side="left")
        
        # Queue list card
        queue_card = tk.Frame(xml_card, bg=self.colors['bg_main'], relief="flat")
        queue_card.pack(fill="both", expand=True, padx=20, pady=(0, 15))
        
        tk.Label(queue_card, text="Files in Queue", font=self.fonts['body'], 
                bg=self.colors['bg_main'], fg=self.colors['text_primary']).pack(anchor="w", pady=(0, 8))
        
        # Listbox container
        listbox_container = tk.Frame(queue_card, bg=self.colors['bg_card'], relief="solid", bd=1)
        listbox_container.pack(fill="both", expand=True)
        
        # Scrollbar
        xml_scrollbar = tk.Scrollbar(listbox_container, bg=self.colors['bg_card'])
        xml_scrollbar.pack(side="right", fill="y")
        
        # Listbox
        self.xml_queue_listbox = tk.Listbox(listbox_container, yscrollcommand=xml_scrollbar.set, 
                                           height=8, font=self.fonts['body'],
                                           bg=self.colors['bg_card'], fg=self.colors['text_primary'],
                                           selectbackground=self.colors['bg_primary'],
                                           selectforeground='white',
                                           relief='flat', bd=0, highlightthickness=0)
        self.xml_queue_listbox.pack(side="left", fill="both", expand=True, padx=2, pady=2)
        xml_scrollbar.config(command=self.xml_queue_listbox.yview)

        # Action button
        action_frame = tk.Frame(xml_card, bg=self.colors['bg_card'])
        action_frame.pack(fill="x", padx=20, pady=(0, 15))
        
        self.batch_generate_btn = self.create_modern_button(action_frame, 
                                                           "Process All Files in Queue", 
                                                           self.process_xml_queue,
                                                           bg_color=self.colors['bg_success'],
                                                           width=25)
        self.batch_generate_btn.pack(pady=5)
        
        # Progress and status
        self.progress_xml = ttk.Progressbar(xml_card, mode="determinate", style="Custom.Horizontal.TProgressbar")
        self.progress_xml.pack(pady=(0, 10), padx=20, fill="x")
        self.progress_xml.pack_forget()

        self.xml_status_label = tk.Label(xml_card, text="", font=self.fonts['small'], 
                                         bg=self.colors['bg_card'], fg=self.colors['accent'])
        self.xml_status_label.pack(pady=(0, 15))

    def create_data_frame(self):
        """Create modern Data TXT file frame"""
        data_card = tk.Frame(self.data_frame, bg=self.colors['bg_card'], relief="flat")
        data_card.pack(fill="both", expand=True, pady=(0, 15))
        
        header_frame = tk.Frame(data_card, bg=self.colors['bg_card'], height=50)
        header_frame.pack(fill="x", padx=20, pady=(15, 10))
        header_frame.pack_propagate(False)
        
        tk.Label(header_frame, text="Data TXT Files", font=self.fonts['heading'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).pack(side="left", anchor="w")
        
        self.data_queue_label = tk.Label(header_frame, text="0 files in queue", 
                                         font=self.fonts['small'], bg=self.colors['bg_card'], 
                                         fg=self.colors['text_secondary'])
        self.data_queue_label.pack(side="right", anchor="e")
        
        controls_frame = tk.Frame(data_card, bg=self.colors['bg_card'])
        controls_frame.pack(fill="x", padx=20, pady=(0, 15))
        
        self.add_data_btn = self.create_modern_button(controls_frame, "Add Files to Queue", 
                                                     self.add_data_to_queue, 
                                                     bg_color=self.colors['bg_primary'])
        self.add_data_btn.pack(side="left", padx=(0, 10))
        
        clear_btn = tk.Button(controls_frame, text="Clear Queue", command=self.clear_data_queue,
                            font=self.fonts['body'], bg=self.colors['bg_card'], 
                            fg=self.colors['text_secondary'], activebackground=self.colors['bg_main'],
                            activeforeground=self.colors['text_primary'], relief='flat', bd=0,
                            padx=15, pady=10, cursor='hand2')
        ...
        queue_card = tk.Frame(data_card, bg=self.colors['bg_main'], relief="flat")
        queue_card.pack(fill="both", expand=True, padx=20, pady=(0, 15))
        ...
        data_scrollbar = tk.Scrollbar(listbox_container, bg=self.colors['bg_card'])
        data_scrollbar.pack(side="right", fill="y")
        
        self.data_queue_listbox = tk.Listbox(listbox_container, yscrollcommand=data_scrollbar.set, 
                                             height=8, font=self.fonts['body'],
                                             bg=self.colors['bg_card'], fg=self.colors['text_primary'],
                                             selectbackground=self.colors['bg_primary'],
                                             selectforeground='white',
                                             relief='flat', bd=0, highlightthickness=0)
        self.data_queue_listbox.pack(side="left", fill="both", expand=True, padx=2, pady=2)
        data_scrollbar.config(command=self.data_queue_listbox.yview)

        action_frame = tk.Frame(data_card, bg=self.colors['bg_card'])
        action_frame.pack(fill="x", padx=20, pady=(0, 15))
        
        self.batch_generate_data_btn = self.create_modern_button(action_frame, 
                                                                  "Process All Files in Queue", 
        ...
        self.progress_data = ttk.Progressbar(data_card, mode="determinate", style="Custom.Horizontal.TProgressbar")
        self.progress_data.pack(pady=(0, 10), padx=20, fill="x")
        self.progress_data.pack_forget()

        self.data_status_label = tk.Label(data_card, text="", font=self.fonts['small'], 
                                         bg=self.colors['bg_card'], fg=self.colors['accent'])
        self.data_status_label.pack(pady=(0, 15))

    def create_idfx_frame(self):
        """Create modern IDFX file frame"""
        idfx_card = tk.Frame(self.idfx_frame, bg=self.colors['bg_card'], relief="flat")
        idfx_card.pack(fill="both", expand=True, pady=(0, 15))
        
        header_frame = tk.Frame(idfx_card, bg=self.colors['bg_card'], height=50)
        header_frame.pack(fill="x", padx=20, pady=(15, 10))
        header_frame.pack_propagate(False)
        
        tk.Label(header_frame, text="IDFX Files", font=self.fonts['heading'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).pack(side="left", anchor="w")
        
        self.idfx_queue_label = tk.Label(header_frame, text="0 files in queue", 
                                         font=self.fonts['small'], bg=self.colors['bg_card'], 
                                         fg=self.colors['text_secondary'])
        self.idfx_queue_label.pack(side="right", anchor="e")
        
        controls_frame = tk.Frame(idfx_card, bg=self.colors['bg_card'])
        controls_frame.pack(fill="x", padx=20, pady=(0, 15))
        
        self.add_idfx_btn = self.create_modern_button(controls_frame, "Add Files to Queue", 
                                                      self.add_idfx_to_queue, 
                                                      bg_color=self.colors['bg_primary'])
        self.add_idfx_btn.pack(side="left", padx=(0, 10))
        
        clear_btn = tk.Button(controls_frame, text="Clear Queue", command=self.clear_idfx_queue,
                            font=self.fonts['body'], bg=self.colors['bg_card'], 
                            fg=self.colors['text_secondary'], activebackground=self.colors['bg_main'],
                            activeforeground=self.colors['text_primary'], relief='flat', bd=0,
                            padx=15, pady=10, cursor='hand2')
        ...
        queue_card = tk.Frame(idfx_card, bg=self.colors['bg_main'], relief="flat")
        queue_card.pack(fill="both", expand=True, padx=20, pady=(0, 15))
        ...
        idfx_scrollbar = tk.Scrollbar(listbox_container, bg=self.colors['bg_card'])
        idfx_scrollbar.pack(side="right", fill="y")
        
        self.idfx_queue_listbox = tk.Listbox(listbox_container, yscrollcommand=idfx_scrollbar.set, 
                                            height=8, font=self.fonts['body'],
                                            bg=self.colors['bg_card'], fg=self.colors['text_primary'],
                                            selectbackground=self.colors['bg_primary'],
                                            selectforeground='white',
                                            relief='flat', bd=0, highlightthickness=0)
        self.idfx_queue_listbox.pack(side="left", fill="both", expand=True, padx=2, pady=2)
        idfx_scrollbar.config(command=self.idfx_queue_listbox.yview)

        action_frame = tk.Frame(idfx_card, bg=self.colors['bg_card'])
        action_frame.pack(fill="x", padx=20, pady=(0, 15))
        
        self.batch_generate_idfx_btn = self.create_modern_button(action_frame, 
                                                                  "Process All Files in Queue", 
                                                                  self.process_idfx_queue,
                                                                  bg_color=self.colors['bg_success'],
                                                                  width=25)
        self.batch_generate_idfx_btn.pack(pady=5)
        
        self.progress_idfx = ttk.Progressbar(idfx_card, mode="determinate", style="Custom.Horizontal.TProgressbar")
        self.progress_idfx.pack(pady=(0, 10), padx=20, fill="x")
        self.progress_idfx.pack_forget()

        self.idfx_status_label = tk.Label(idfx_card, text="", font=self.fonts['small'], 
                                         bg=self.colors['bg_card'], fg=self.colors['accent'])
        self.idfx_status_label.pack(pady=(0, 15))

    def create_shared_settings(self):
        """Create modern shared settings section"""
        # Create settings container with card style
        settings_container = tk.Frame(self.main_frame, bg=self.colors['bg_main'])
        settings_container.pack(fill="x", pady=(20, 0))
        
        # Text Settings Card
        self.create_text_settings_card(settings_container)
        
        # Moving Window Card
        self.create_moving_window_card(settings_container)
        
        # Speed Settings Card
        self.create_speed_settings_card(settings_container)
        
        # Timing Controls Card
        self.create_timing_controls_card(settings_container)
        
        # Options Card
        self.create_options_card(settings_container)

    def create_text_settings_card(self, parent):
        """Create text settings card"""
        card = tk.Frame(parent, bg=self.colors['bg_card'], relief="flat")
        card.pack(fill="x", pady=(0, 15))
        
        header = tk.Frame(card, bg=self.colors['bg_card'], height=40)
        header.pack(fill="x", padx=20, pady=(15, 10))
        header.pack_propagate(False)
        
        tk.Label(header, text="Text Settings", font=self.fonts['heading'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).pack(side="left", anchor="w")
        ...
        # Row 1
        tk.Label(content, text="Font:", font=self.fonts['body'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).grid(row=0, column=0, sticky="w", padx=(0, 10), pady=8)
        
        self.font_families = sorted(set(tkfont.families()))
        self.font_family_var = tk.StringVar(value="Arial")
        self.font_family_menu = ttk.Combobox(content, textvariable=self.font_family_var, 
                                            values=self.font_families, state="readonly", 
                                            width=18, font=self.fonts['body'])
        self.font_family_menu.grid(row=0, column=1, padx=(0, 20), pady=8)
        
        tk.Label(content, text="Size:", font=self.fonts['body'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).grid(row=0, column=2, sticky="w", padx=(0, 10), pady=8)
        
        self.font_size_var = tk.IntVar(value=30)
        self.font_size_entry = tk.Entry(content, textvariable=self.font_size_var, width=8, 
                                       font=self.fonts['body'], relief="solid", bd=1)
        self.font_size_entry.grid(row=0, column=3, padx=(0, 20), pady=8)

        self.bold_var = tk.BooleanVar(value=True)
        self.bold_check = tk.Checkbutton(content, text="Bold", variable=self.bold_var,
                                        font=self.fonts['body'], bg=self.colors['bg_card'],
                                        fg=self.colors['text_primary'], selectcolor=self.colors['bg_card'])
        self.bold_check.grid(row=0, column=4, sticky="w", pady=8)
        
        # Row 2
        tk.Label(content, text="Margin:", font=self.fonts['body'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).grid(row=1, column=0, sticky="w", padx=(0, 10), pady=8)
        
        self.margin_var = tk.IntVar(value=20)
        self.margin_entry = tk.Entry(content, textvariable=self.margin_var, width=8, 
                                    font=self.fonts['body'], relief="solid", bd=1)
        self.margin_entry.grid(row=1, column=1, sticky="w", pady=8)

    def create_moving_window_card(self, parent):
        """Create moving window settings card"""
        card = tk.Frame(parent, bg=self.colors['bg_card'], relief="flat")
        card.pack(fill="x", pady=(0, 15))
        ...
        tk.Label(header, text="Moving Window", font=self.fonts['heading'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).pack(side="left", anchor="w")
        ...
        self.moving_window_check = tk.Checkbutton(content, text="Enable Moving Window", 
                                                variable=self.moving_window_var, 
                                                command=self.update_window_controls,
                                                font=self.fonts['body'], 
                                                bg=self.colors['bg_card'],
                                                fg=self.colors['text_primary'],
                                                selectcolor=self.colors['bg_card'])
        self.moving_window_check.grid(row=0, column=0, sticky="w", pady=8, columnspan=2)
        
        tk.Label(content, text="Window Size (chars):", font=self.fonts['body'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).grid(row=1, column=0, sticky="w", padx=(0, 10), pady=8)
        self.window_size_var = tk.IntVar(value=10)
        self.window_size_entry = tk.Entry(content, textvariable=self.window_size_var, width=8, 
                                         state="disabled", font=self.fonts['body'], relief="solid", bd=1)
        self.window_size_entry.grid(row=1, column=1, sticky="w", padx=(0, 20), pady=8)
        
        self.window_wordonly_var = tk.BooleanVar(value=False)
        self.window_wordonly_check = tk.Checkbutton(content, text="Window Only Current Word", 
                                                   variable=self.window_wordonly_var, state="disabled",
                                                   font=self.fonts['body'], bg=self.colors['bg_card'],
                                                   fg=self.colors['text_primary'], selectcolor=self.colors['bg_card'])
        self.window_wordonly_check.grid(row=1, column=2, sticky="w", pady=8)
        
        tk.Label(content, text="Mask Character:", font=self.fonts['body'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).grid(row=2, column=0, sticky="w", padx=(0, 10), pady=8)
        self.mask_character_var = tk.StringVar(value="_")
        self.mask_character_entry = tk.Entry(content, textvariable=self.mask_character_var, width=5, 
                                             state="disabled", font=self.fonts['body'], relief="solid", bd=1)
        self.mask_character_entry.grid(row=2, column=1, sticky="w", pady=8)

    def create_speed_settings_card(self, parent):
        """Create speed settings card"""
        card = tk.Frame(parent, bg=self.colors['bg_card'], relief="flat")
        card.pack(fill="x", pady=(0, 15))
        ...
        tk.Label(header, text="Speed Settings", font=self.fonts['heading'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).pack(side="left", anchor="w")
        ...
        # Row 1
        tk.Label(content, text="Video Speed Multiplier:", font=self.fonts['body'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).grid(row=0, column=0, sticky="w", padx=(0, 10), pady=8)
        self.video_speed_var = tk.DoubleVar(value=1.0)
        self.video_speed_entry = tk.Entry(content, textvariable=self.video_speed_var, width=8, 
                                         font=self.fonts['body'], relief="solid", bd=1)
        self.video_speed_entry.grid(row=0, column=1, padx=(0, 20), pady=8)
        
        tk.Label(content, text="Word Speed (s/word):", font=self.fonts['body'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).grid(row=0, column=2, sticky="w", padx=(0, 10), pady=8)
        self.word_speed_var = tk.DoubleVar(value=0.15)
        self.word_speed_entry = tk.Entry(content, textvariable=self.word_speed_var, width=8, 
                                        font=self.fonts['body'], relief="solid", bd=1)
        self.word_speed_entry.grid(row=0, column=3, padx=(0, 20), pady=8)
        
        tk.Label(content, text="Space Duration (s):", font=self.fonts['body'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).grid(row=0, column=4, sticky="w", padx=(0, 10), pady=8)
        self.space_duration_var = tk.DoubleVar(value=0.25)
        self.space_duration_entry = tk.Entry(content, textvariable=self.space_duration_var, width=8, 
                                            font=self.fonts['body'], relief="solid", bd=1)
        self.space_duration_entry.grid(row=0, column=5, pady=8)
        
        # Uniform typing mode
        uniform_frame = tk.Frame(content, bg=self.colors['bg_card'])
        uniform_frame.grid(row=1, column=0, columnspan=6, sticky="w", pady=8)
        
        self.uniform_typing_var = tk.BooleanVar(value=False)
        self.uniform_typing_check = tk.Checkbutton(uniform_frame, text="Uniform Typing Speed", 
                                                   variable=self.uniform_typing_var,
                                                   font=self.fonts['body'], bg=self.colors['bg_card'],
                                                   fg=self.colors['text_primary'], selectcolor=self.colors['bg_card'])
        self.uniform_typing_check.pack(side="left", padx=(0, 15))
        
        tk.Label(uniform_frame, text="Characters per Second:", font=self.fonts['body'], 
        ...
        self.chars_per_sec_entry = tk.Entry(uniform_frame, textvariable=self.chars_per_sec_var, width=8, 
                                           font=self.fonts['body'], relief="solid", bd=1)
        self.chars_per_sec_entry.pack(side="left")

    def create_timing_controls_card(self, parent):
        """Create timing controls card"""
        card = tk.Frame(parent, bg=self.colors['bg_card'], relief="flat")
        card.pack(fill="x", pady=(0, 15))
        ...
        tk.Label(header, text="Video Timing Controls", font=self.fonts['heading'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).pack(side="left", anchor="w")
        ...
        self.enable_timing_check = tk.Checkbutton(content, text="Enable Video Timing Controls", 
                                                  variable=self.enable_timing_var, 
                                                  command=self.update_timing_controls,
                                                  font=self.fonts['body'], bg=self.colors['bg_card'],
                                                  fg=self.colors['text_primary'], selectcolor=self.colors['bg_card'])
        self.enable_timing_check.grid(row=0, column=0, sticky="w", pady=8, columnspan=4)
        
        tk.Label(content, text="Start Time (ms):", font=self.fonts['body'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).grid(row=1, column=0, sticky="w", padx=(0, 10), pady=8)
        self.start_time_var = tk.IntVar(value=0)
        self.start_time_entry = tk.Entry(content, textvariable=self.start_time_var, width=12, 
                                        state="disabled", font=self.fonts['body'], relief="solid", bd=1)
        self.start_time_entry.grid(row=1, column=1, padx=(0, 20), pady=8)
        
        tk.Label(content, text="End Time (ms):", font=self.fonts['body'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).grid(row=1, column=2, sticky="w", padx=(0, 10), pady=8)
        self.end_time_var = tk.IntVar(value=0)
        self.end_time_entry = tk.Entry(content, textvariable=self.end_time_var, width=12, 
                                      state="disabled", font=self.fonts['body'], relief="solid", bd=1)
        self.end_time_entry.grid(row=1, column=3, pady=8)
        
        tk.Label(content, text="Duration (%):", font=self.fonts['body'], 
                bg=self.colors['bg_card'], fg=self.colors['text_primary']).grid(row=2, column=0, sticky="w", padx=(0, 10), pady=8)
        self.duration_percent_var = tk.DoubleVar(value=100.0)
        self.duration_percent_entry = tk.Entry(content, textvariable=self.duration_percent_var, width=12, 
                                               state="disabled", font=self.fonts['body'], relief="solid", bd=1)
        self.duration_percent_entry.grid(row=2, column=1, sticky="w", padx=(0, 20), pady=8)
        
        self.timing_mode_var = tk.StringVar(value="absolute")
        timing_mode_frame = tk.Frame(content, bg=self.colors['bg_card'])
        timing_mode_frame.grid(row=2, column=2, columnspan=2, sticky="w", pady=8)
        tk.Radiobutton(timing_mode_frame, text="Absolute Time", variable=self.timing_mode_var, 
                      value="absolute", command=self.update_timing_mode,
                      font=self.fonts['body'], bg=self.colors['bg_card'],
                      fg=self.colors['text_primary'], selectcolor=self.colors['bg_card']).pack(side="left", padx=(0, 15))
        tk.Radiobutton(timing_mode_frame, text="Percentage", variable=self.timing_mode_var, 
                      value="percentage", command=self.update_timing_mode,
                      font=self.fonts['body'], bg=self.colors['bg_card'],
                      fg=self.colors['text_primary'], selectcolor=self.colors['bg_card']).pack(side="left")
    ...
    def create_options_card(self, parent):
        ...
        self.save_video_check = tk.Checkbutton(content, text="Save Video", variable=self.save_video_var,
                                               font=self.fonts['body'], bg=self.colors['bg_card'],
                                               fg=self.colors['text_primary'], selectcolor=self.colors['bg_card'])
        self.save_video_check.pack(side="left", padx=(0, 15))
        
        self.preview_btn = tk.Button(content, text="Preview Video", command=self.preview_video,
                                    font=self.fonts['button'], bg=self.colors['bg_secondary'], fg='white',
                                    activebackground='#6E6E73', activeforeground='white',
                                    relief='flat', bd=0, padx=20, pady=10, cursor='hand2')
        self.preview_btn.pack(side="left", padx=(0, 15))
        
        self.save_settings_btn = self.create_modern_button(content, "Save Settings", 
                                                          self.save_settings,
                                                          bg_color=self.colors['bg_primary'])
    ...
    def on_file_type_change(self, event=None):
        ...
        # Update scroll region after frame change
    ...
    def update_xml_queue_display(self):
        ...
        self.xml_queue_label.config(text=f"{count} files in queue")
    ...
    def update_data_queue_display(self):
        ...
        self.data_queue_label.config(text=f"{count} files in queue")
    ...
    def update_idfx_queue_display(self):
        ...
        self.idfx_queue_label.config(text=f"{count} files in queue")
